...
- **REQ_E-012**: The application will provide placeholder content internally, without requiring user-supplied template files. (Amended for Source-Driven Generation, §20: a user may supply a Source CSV or Directory Template to define record paths and File Types, but source file bytes are never copied — all generated content remains internal placeholder content.)
...
- **REQ-001**: When `--with-metadata` is specified, the output `.dat` file must include the additional columns: `Custodian`, `Date Sent`, `Author`, `File Size`, and `Compression Method` (the Archive-wide `--compression` setting: `Store`, `Deflate`, `Deflate64`, or `BZip2`). This applies to the Standard and Loadfile-Only DAT, which carry the Metadata column set; the OPT format carries no Metadata columns and the Production Set DAT uses its own fixed schema, so neither includes `Compression Method` under any flag. Note: For the Email File Type, email-specific metadata columns are always included regardless of this flag, as these are intrinsic to Emails. `Compression Method` is **Email-intrinsic**: it is part of Email Metadata, so an Archive containing at least one Email includes the column whether or not `--with-metadata` is specified, subject to the format exclusions above. For every other File Type the column is strictly gated on `--with-metadata`.
...
- **REQ-022**: The application must exit with a validation error if `--target-zip-size` is specified without the `--count` argument. (Amended for Source-Driven Generation, §20: with `--input-csv`/`--directory-template`, the Source Record count satisfies this requirement; padding is distributed across the source File Types.)
...
- **REQ-039**: New optional command-line arguments for Bates numbering shall be introduced:
  - `--bates-prefix <prefix>`: Prefix for Bates numbering (e.g., "CLIENT001"), maximum 200 UTF-8 bytes per element
...
- `--with-metadata`: (Optional) Generates a Load File with additional metadata columns (Custodian, Date Sent, Author, File Size, Compression Method). Compression Method is additionally Email-intrinsic and appears for Email records without this flag; see REQ-001 for the per-format exclusions.
...
- `--bates-prefix <prefix>`: (Optional) Prefix for Bates numbering, maximum 200 UTF-8 bytes per element.
...
- `--seed <number>`: (Optional) Random seed for reproducible data generation. For Archive Test workflows, the value must be a non-negative integer and defaults to 42.
...
- **REQ-125**: Any unknown CLI argument or unconsumed value shall cause the parsing phase to fail immediately with a clear error message and a non-zero exit code, except that the `--benchmark` and `--chaos-list` early-exit flags bypass parsing validation.
...
- **REQ-157**: Zipper shall support comparing two or more Production Manifests. With three or more manifests, the final resolved Production Manifest is the new Production Manifest and every preceding resolved Production Manifest is prior input; the Comparison Report preserves the supplied manifest order.
- **REQ-158**: Comparison reports shall support replacement workflows. Matching and classification are positional-mode-independent for replacement and reproduction (REQ-160): new-side records match prior-side records by Bates Number (case-insensitive), then by Control Number (case-insensitive). A Bates match with equal case-insensitive path, equal case-insensitive Control Number, and no conflicting non-empty hash is `unchanged`; a Bates match with a differing path, Control Number, or non-empty hash is `changed`; a Control-Number-only match is `replaced`; records present only on the new side are `added`; records present only on the prior side are `removed`. Empty hash values never block an `unchanged` classification. Duplicate Bates/Control Numbers within one side are reported as `duplicated` (REQ-161) without affecting per-record classification.
- **REQ-159**: Comparison reports shall support supplemental workflows.
- **REQ-160**: Comparison reports shall support reproduction workflows. Reproduction reuses the replacement matching and classification rules (REQ-158) and differs only in workflow intent and the reported Comparison Mode; no reproduction-specific classification branch exists.
...
- **REQ-162**: The Comparison Report shall include Bates Number range analysis by Production Set and Volume, with skipped Bates totals (`totalSkippedBates`) supporting 64-bit non-negative integer values above `Int32.MaxValue` and checked arithmetic. A Volume present only in the New Production Manifest has status `added`; a Volume present only in a Prior Production Manifest has status `removed`. A same-named Volume has status `changed` when its Bates Number membership, Bates Number range, record count, matched Native File path, or conflicting non-empty hash differs; otherwise it has status `unchanged`. Volume names, Bates Numbers, Native File paths, and hashes are compared case-insensitively; input order alone and an empty hash on either side do not mark a Volume as changed.
...
This section pins the public CLI contract for each rolling, supplemental, and comparison option: argument name, default, cardinality and list syntax, accepted values, dependencies and conflicts, early-exit behavior, and output selection. These contracts are immutable and must agree with the README Arguments Quick Reference, the help text emitted by `HelpTextGenerator`, the `CliModule` `TryApply`/`TryBuild` enforcement, the `CrossCuttingRules` cross-domain checks, and the `Pipeline.Build` wiring.

### Rolling Production Set Contracts

- **REQ-170**: `--production-id <string|list>` shall set the Production ID used in Production Set output directory names, Production Manifest Metadata, and report identifiers. Requires `--production-set`; supplying the flag without `--production-set` shall fail validation before output generation with a clear error message and non-zero exit code. Accepts a single value or a comma-separated list. A wholly empty supplied value is a supplied value, not an omission, and shall fail validation before output generation with a clear error message and non-zero exit code; the timestamp default applies only when the flag is omitted. Because each Production ID becomes exactly one directory-name segment beneath `--output-path`, every supplied and derived element must be a Safe Path Segment: path separators, the dot segments `.` and `..`, control characters, a trailing space or dot, and the characters Windows reserves in file names (`: * ? " < > |`) shall fail validation before output generation with a clear error message and non-zero exit code. A reserved DOS device name (`CON`, `PRN`, `AUX`, `NUL`, `CONIN$`, `CONOUT$`, `COM1`–`COM9`, `LPT1`–`LPT9`, where the COM/LPT suffix may also be the superscript one, two, or three, with or without an extension) is likewise not a Safe Path Segment. The rejected set is fixed rather than read from the host platform's invalid-file-name or device list, so the same value is accepted or rejected identically on every platform. The Safe Path Segment rule is enforced at the CLI input seam; components below the seam treat the Production ID as already validated. Each generated Production ID element must not exceed 250 UTF-8 bytes. The limit is validated after any rolling Production ID derivation, so a derived suffix such as `_002` counts toward the element's limit, and oversized elements fail validation before output generation in every mode in which `--production-id` is supplied. When a supplied value violates more than one rule, the rules are evaluated in this fixed order and the first failure is reported: wholly empty value, 250-byte limit, missing `--production-set`, Safe Path Segment, entry count versus `--rolling-count`, duplicate Production IDs, then empty entry within a list. When omitted, the Production ID defaults to `PRODUCTION_<yyyyMMdd_HHmmss>` for a single Production Set, and `PRODUCTION_<yyyyMMdd_HHmmss>_<NNN>` (zero-padded sequence) for rolling Production Sets. When a single value is supplied with `--rolling-count > 1`, subsequent IDs are derived by incrementing a trailing numeric suffix (preserving width) or by appending `_<N>` when no trailing numeric suffix exists. The number of supplied list entries must equal `--rolling-count` when more than one entry is supplied; duplicate Production IDs (case-insensitive) shall fail validation before output generation with a clear error message and non-zero exit code, as shall an empty entry within a list.
- **REQ-171**: `--rolling-count <number>` shall set the number of Production Sets generated from one Generation Request. Requires `--production-set`. Defaults to `1`. The value must be a positive integer; non-numeric, zero, and negative values shall fail parsing or validation immediately with a clear error message and non-zero exit code.
- **REQ-172**: `--rolling-bates-mode <continuous|restart>` shall set the Bates Number range behavior across rolling Production Sets. Requires `--production-set`. Defaults to `continuous`. `continuous` produces a single contiguous Bates Number range across all rolling Production Sets; `restart` resets the Bates Number range per Production Set. Values are matched case-insensitively; any other value shall fail validation with a clear error message and non-zero exit code. In `continuous` mode, overlapping Bates Number ranges for the same Bates Prefix shall fail validation before output generation.

### Supplemental Production Set Contracts

- **REQ-173**: `--supplemental-production` (parameterless flag) shall enable supplemental Production Set generation mode. Requires `--production-set` and `--prior-manifest`. Conflicts with `--loadfile-only` (inherited from `--production-set`). Specifying `--supplemental-production` without `--production-set` or without `--prior-manifest` shall fail validation before output generation with a clear error message and non-zero exit code.
- **REQ-174**: `--prior-manifest <paths>` shall accept a comma-separated list of paths to prior Production Manifest files (or directories containing a `_manifest.json`) used to validate Bates Number ranges during supplemental generation. Requires `--supplemental-production`. Empty after trimming is invalid. Specifying `--prior-manifest` without `--supplemental-production` shall fail validation with a clear error message and non-zero exit code. The resolved prior Production Manifests must be compatible with the current Bates Prefix and Bates Digits; otherwise validation shall fail before writing output.
- **REQ-175**: `--supplemental-gap-policy <reject|allow>` shall set the gap policy for supplemental generation. Requires `--supplemental-production`. Defaults to `reject`. `reject` fails validation when skipped Bates Numbers are detected between prior and new Production Manifests; `allow` reports the gaps but proceeds with generation. Values are matched case-insensitively; any other value shall fail validation with a clear error message and non-zero exit code. Specifying `--supplemental-gap-policy` without `--supplemental-production` shall fail validation with a clear error message.

### Production Manifest Comparison Contracts

- **REQ-176**: `--compare-production-manifests <paths>` shall accept a comma-separated list of paths to two or more Production Manifest files (or directories containing a `_manifest.json`) to compare. Requires `--comparison-mode` and `--comparison-output`. Entries are processed in supplied order after REQ-180 trimming and empty-entry removal: the final resolved Production Manifest is the new Production Manifest and every preceding resolved Production Manifest is prior input (see REQ-157). When specified, the application shall bypass normal Native File generation and post-generation validation, dispatch directly to the Production Manifest comparison pipeline, and exit. Fewer than two resolvable Production Manifest paths shall fail before output generation. A missing value shall fail parsing with a clear error message and non-zero exit code.
- **REQ-177**: `--comparison-mode <replacement|supplemental|reproduction>` shall select the comparison ruleset applied by the Production Manifest comparison pipeline. Requires `--compare-production-manifests`. Has no default when `--compare-production-manifests` is specified; omitting it shall fail validation with a clear error message and non-zero exit code. Values are matched case-insensitively; any other value shall fail validation with a clear error message and non-zero exit code. `replacement` compares prior vs. new Production Manifests for a replacement workflow, `supplemental` for a supplemental workflow, and `reproduction` for a reproduction workflow (auditing corrected Native File content/Metadata). Specifying `--comparison-mode` without `--compare-production-manifests` shall fail validation with a clear error message.
- **REQ-178**: `--comparison-output <path>` shall set the output file path for the machine-readable comparison JSON report (with human-readable summary). Requires `--compare-production-manifests`. Has no default when `--compare-production-manifests` is specified; omitting it shall fail validation with a clear error message and non-zero exit code. The comparison pipeline shall write the report to this path and exit `0` on a completed comparison; unrecoverable errors (e.g., unresolvable manifest paths, malformed JSON, fewer than two resolvable manifests) shall propagate as a non-zero exit code before a report is written. Specifying `--comparison-output` without `--compare-production-manifests` shall fail validation with a clear error message.
- **REQ-179**: The comparison workflow (`--compare-production-manifests` with `--comparison-mode` and `--comparison-output`) shall short-circuit before `--type`, `--count`, `--output-path`, and Production Set argument validation, because no Native Files are generated. Rolling, supplemental, and redacted Production arguments have no effect in comparison mode and are not evaluated.
- **REQ-180**: Comma-separated list arguments shall be split on `,` with entries trimmed. For `--prior-manifest` and `--compare-production-manifests`, empty entries are removed and at least two entries must remain after trimming (for `--compare-production-manifests`); otherwise the request shall fail before output generation. For `--production-id`, empty entries are not removed and shall fail validation per REQ-170, and the list length must satisfy the cardinality rule in REQ-170.

## 18. Collection Metadata

### FR-025: Collection Metadata Columns

- **REQ-181**: `--with-collection-metadata` (parameterless flag) shall include five Collection Metadata columns in the Load File: Data Source, Collection Date, De-Nisted, Dedupe Group ID, and Processing Status. Defaults to `false` (columns omitted). The flag is independent of `--with-metadata`; both may be specified simultaneously and each adds its own columns.
- **REQ-182**: Collection Metadata column values shall be synthetically generated per record: Data Source drawn from a fixed set of eight sources (Email Server, Network Share, Cloud Storage, Mobile Device, Laptop, Desktop, Server, External Media); Collection Date as a date within the 30 days preceding generation; De-Nisted as `YES` (approximately 85%) or `NO` (approximately 15%); Dedupe Group ID as `GRP` followed by a zero-padded six-digit number (1–999); Processing Status drawn from a fixed set of six statuses (Processed, Pending Review, Approved, Privileged, Redacted, Produced).
- **REQ-183**: Collection Metadata value generation shall be deterministic when `--seed` is provided, producing the same column values for the same seed, File Count, and generation mode.
- **REQ-184**: Collection Metadata columns shall be included in Standard mode for the DAT, CSV, and Concordance Load File Formats. The columns are NOT included in the OPT format (page-level image references) or the EDRM-XML format.
- **REQ-185**: In Loadfile-Only mode, Collection Metadata columns shall be included only when `--column-profile` is also specified; the five columns are merged into the loaded profile. Without `--column-profile`, the flag is silently ignored in Loadfile-Only mode (no columns added, no error).
...
### FR-026: File Type Mix (`--types`)

- **REQ-190**: A new optional argument `--types <type:weight,...>` shall define a File Type Mix: a comma-separated list of `type:weight` pairs where each type is a supported File Type (REQ_E-010) and each weight is a positive integer not exceeding 1,000,000. Types are matched case-insensitively and normalized to lowercase; whitespace around entries is trimmed. Empty entries, malformed pairs, non-numeric/non-positive/oversized weights, unknown types, and duplicate types shall fail validation with a clear error message and non-zero exit code. `--types` is mutually exclusive with `--type` and is not supported with `--loadfile-only` or `--column-profile` (profile-driven columns bypass per-record File Type gating); any violation shall fail validation before output generation. When `--types` is specified, the `--type` requirement (REQ_E-010 argument rule) is satisfied.
- **REQ-191**: File Count shall be allocated across the File Type Mix proportionally by weight using the largest-remainder method, so per-type counts always sum exactly to the requested File Count (never over- or underproduces). File Types are assigned to contiguous 1-based file index ranges in declared order. Allocation is deterministic: identical arguments always produce the identical per-index assignment, independent of `--seed`.
- **REQ-192**: Each Native File's assigned File Type shall drive per-record behavior: the file generator selection, the Native File extension in the Archive or Production Set, and all type-dependent Load File values. Single-File-Type output (no `--types`, or a single-entry mix) shall remain byte-identical to prior behavior.
- **REQ-193**: Email Metadata columns shall be included in the Load File when the Email File Type participates in the run (via `--type eml` or a mix containing `eml`). In a File Type Mix, Email Metadata values shall be populated only on Email records and shall be blank for all other File Types; Attachments and family relationships (`--with-families`) apply only to Email records.
- **REQ-194**: The Page Count column shall be included when the TIFF File Type participates in the run and `--tiff-pages` is specified. In a File Type Mix, Page Count values shall be populated only on TIFF records and shall be blank for all other File Types. OPT page expansion likewise applies only to TIFF records (all other records expand to a single page entry).
- **REQ-195**: In Production Set mode, the DAT `FILE_TYPE` column shall report each record's own File Type (parent rows: the record's assigned type; child rows: the Attachment's extension). In Standard mode, the DAT, CSV, and Concordance Load Files shall include a File Type column (`File Type` in DAT/CSV, `FILE_TYPE` in Concordance) only when a File Type Mix is active; single-type Standard Load Files remain unchanged.
- **REQ-196**: When `--load-file-format`/`--load-file-formats` is not explicitly specified, the default Load File Formats shall be DAT+OPT if any participating File Type is `jpg` or `tiff` (unchanged single-type rule: `--type jpg`/`--type tiff` alone), otherwise DAT. A single-entry mix (e.g., `--types "eml:1"`) shall behave exactly as the equivalent `--type` argument.

## 20. Source-Driven Generation

### FR-027: Source-Driven Generation (`--input-csv`, `--directory-template`)

- **REQ-197**: A new optional argument `--input-csv <path>` shall read a Source CSV whose rows define one record each. Required columns are `FilePath` and `FileType`; optional columns are `ControlNumber` (alias `DocId`) and `BatesNumber` (aliases `Bates`, `BegBates`); every other column becomes Source Metadata. Header matching is case-insensitive and ignores spaces, underscores, and hyphens. Parsing follows standard CSV quoting rules (quoted fields, escaped double-quotes, embedded newlines), tolerates a UTF-8 BOM, and skips blank lines. Missing required columns, duplicate columns, duplicate paths, empty files, unknown File Types, unsafe paths (REQ-199), and File Type/extension mismatches shall fail with a row-numbered error and non-zero exit code.
- **REQ-198**: A new optional argument `--directory-template <path>` shall walk a directory recursively; each file becomes a Source Record with its relative path as File Path and its File Type inferred from the extension (`.jpg`/`.jpeg` → `jpg`, `.tif`/`.tiff` → `tiff`). Hidden and system entries are included; symbolic links (reparse points) are not followed, to prevent infinite recursion through link cycles. Files with unsupported extensions shall fail validation naming the offending file. Source bytes are never copied into the output; all Native File content remains internally generated placeholder content (REQ_E-012).
- **REQ-199**: `--input-csv` and `--directory-template` shall be mutually exclusive. Source File Paths must be relative and safe: no parent-directory segments (`..`), rooted or UNC paths, drive-qualified paths, dot-segments, invalid characters, reserved Windows device names (`CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9`, `LPT1`-`LPT9`, with or without an extension), or segments ending in a dot or space (unextractable on Windows); the whole path is trimmed of leading/trailing whitespace before these checks. Backslashes are normalized to `/` and repeated separators collapsed. Paths must be unique per record after normalization (case-insensitive). Input files and template directories must be located within the working directory and must exist; violations fail validation with a clear error message and non-zero exit code.
- **REQ-200**: With source input, `--count` becomes optional; when specified it must equal the Source Record count or validation fails. `--type` is not required, and `--type`/`--types` are rejected because File Types come from the source. (Amended for Production Set support: source input is supported with `--production-set` per REQ-205 and REQ-206.)
- **REQ-201**: Each Source Record shall become exactly one Native File (Standard mode) or one Load File record (Loadfile-Only mode), in source order. Archive entry paths and Load File File Path values use the sanitized relative path verbatim, and each record's File Type drives per-record behavior (generator selection, Email Metadata and Page Count gating, OPT page expansion). In Loadfile-Only mode, File Size, Page Count, and simulated hash values remain synthetic. The combined Archive entry namespace of Native Files, generated Attachments, Extracted Text companion files, and included Load Files/Audit Files must not collide; colliding entry paths (compared case-insensitively) shall fail generation descriptively with a non-zero exit code rather than silently skipping or renaming any entry, and any failed run shall clean up only active run artifacts while preserving pre-existing files.
- **REQ-202**: A Source Record's `ControlNumber` value shall override the generated Control Number for that record. A `BatesNumber` value shall override the Bates sequence value for that record; specifying the BatesNumber column requires `--bates-prefix`, and rows without a Bates value fall back to the Bates sequence. Non-empty `ControlNumber` and `BatesNumber` values must each be unique across Source Records (case-insensitive); duplicates fail validation with a row-numbered error, because colliding identities produce duplicate Load File identities and OPT image paths. Explicit values must also stay clear of the identities the run would generate: a `ControlNumber` equal to a generated `DOC{index:D8}` value, or a `BatesNumber` equal to a value of the configured Bates sequence for this run (compared case-insensitively, including padding), shall fail validation with a clear error.
- **REQ-203**: Source Metadata values shall surface in the Load File only through Column Profile mapping (`--column-profile`, DAT format): a profile column whose name matches a Source Metadata key (case-insensitive, ignoring non-alphanumeric characters) takes the source value for that record; unmatched Source Metadata keys are ignored.
- **REQ-204**: When Source Records include more than one File Type, per-record gating and the File Type column shall follow REQ-192 through REQ-195 as if a File Type Mix were declared, and the default Load File Format rule (REQ-196) shall apply to the source File Types (DAT+OPT when any record is `jpg` or `tiff`).
- **REQ-205**: Source input is supported with `--production-set`. Each Source Record becomes one Native File in record order; Volume assignment follows the standard Volume Size split, and Bates Numbers come from the configured Bates sequence (per-record Bates overrides are rejected per REQ-206), keeping Production Manifest Volume ranges exact. The new optional argument `--source-path-mode <bates|preserve|originals>` (default `bates`, valid only together with `--production-set` and source input; any other value fails validation) controls Native File placement: `bates` uses the standard Production Set layout (`NATIVES/VOL###/<bates>.<ext>`, the source path feeds only Load File Metadata); `preserve` keeps source subdirectories under the Volume with a Bates-named file (`NATIVES/VOL###/<source-dirs>/<bates>.<ext>`, root-level entries at the Volume root); `originals` mirrors the full source relative path with the original filename under `ORIGINALS/` and adds an `originals` entry to the Production Manifest's `directories`. TEXT, IMAGES, REDACTED, and Attachment child artifacts stay Volume-rooted and Bates-named in every mode. The built-in Production Set Load File has a fixed Bates-based header (DOCID, BATES_NUMBER, …): a Source Record's Control Number and Source Metadata do not surface in Production Set Load Files (Column Profile mapping per REQ-203 applies to Standard and Loadfile-Only modes only).
- **REQ-206**: With `--production-set`, a source `BatesNumber` column shall fail validation with a clear error and non-zero exit code, because per-record Bates overrides would break exact per-Volume Bates ranges in the Production Manifest.
- **REQ-207**: Source input shall be capped at 10,000,000 Source Records (CSV data rows or directory-template files), because Source Records are held fully in memory. Inputs exceeding the cap shall fail validation with a clear error naming the cap; larger sources must be split into multiple runs. (This bounds, but does not remove, the 100M-file scale expectation of REQ_E-009 for source-driven runs.)
...
- **REQ-208**: A new optional argument `--archive-test-suite <smoke|compatibility|malformed|security|encoding|all>` shall select a predefined Archive Test Fixture suite, and a new optional argument `--archive-test-cases <key,key,...>` shall filter generation to the named Case Keys. Unknown Case Keys, an omitted `--output-path`, or any generation flag combined with these arguments shall fail validation with a clear error and non-zero exit code.
...
- **REQ-210**: The Fixture ID shall be `atc-` plus 64 lowercase SHA-256 hex characters over the canonical LF-terminated descriptor (magic `zipper-archive-test`, descriptor version, generator contract version, Case Key, case revision, expectation revision, Seed, final Archive SHA-256) defined in [docs/archive-test-suites.md](docs/archive-test-suites.md). The same Archive bytes, case, revisions, and Seed shall always reproduce the same Fixture ID; a different case, Seed, revision, or final byte shall change it; output directory and suite selection shall not affect it. Duplicate Fixture IDs shall be detected before any publication. Frozen test vector: the 22-byte empty Archive with Case Key `valid-empty`, Case Revision 1, Expectation Revision 2, Seed 42 yields `atc-4d275f1fe266174e43c71d2cbe42084da23ce42369a17af7b84a9d165b6c489f`.
- **REQ-211**: Archive Test Fixture generation shall accept only non-negative integer values for `--seed`, which defaults to 42. A negative seed shall fail validation before output generation with `Error: --seed must be a non-negative integer.` and a non-zero exit code. The same seed and case definition shall always produce byte-identical fixture Archives and Expectation Files.
- **REQ-212**: Every fixture shall carry a classification of `valid`, `malformed`, or `policy-sensitive`, and its Expectation File shall record per-operation expectations (`list`, `read-entry`, `integrity-check`, `extract`) with named reader profiles, allowed outcomes, allowed failure stages, and invariants. All four operations shall be declared; an Expectation File omitting one shall fail verification. Allowed failure stages bind per reader profile, not across the whole operation: where a profile's record allows a staged outcome, that record's own allowed failure stage set must be non-empty and must contain the stage actually observed, and a sibling profile's stage set does not satisfy it. Where an Expectation File is not scoped to a platform, every operation must have at least one expectation applicable on the host verifying the Archive, so a fixture cannot report every operation as `not-run` and still be counted as verified. A Case Key scoped entirely to platforms other than the verifying host remains a documented skip rather than a pass. `unsupported` and `not-run` shall not count as passing verifications.
- **REQ-213**: Fixture generation shall enforce v1 budgets with checked arithmetic during writes: per-Archive physical size ≤ 16 MiB, recursively expanded recipe content ≤ 32 MiB, ≤ 1,000 entries per Archive, entry path depth ≤ 2, Expectation File JSON ≤ 1 MiB, total output folder ≤ 256 MiB, and a ≤ 10-second per-fixture deadline; one fixture artifact shall be processed at a time. Adversarial declared sizes shall be recorded as decimal strings or raw hex, never as trusted JSON integers, and expansion budgets shall be declared independently of them.
- **REQ-214**: Publication shall never overwrite user data: an existing output directory shall be rejected before anything is written; fixtures shall be staged in an owned sibling directory, created with exclusive-create pairs, and atomically renamed into place; on failure only the owned staging directory shall be cleaned up.
- **REQ-215**: The Archive Test workflow shall exit 0 on success, 1 on validation or generation failure (leaving no partial publication), and 130 on cancellation.
- **REQ-216**: The Archive Test workflow shall be dispatched in `Program` as a short-circuit before `Pipeline.Build`, analogous to the comparison workflow. It shall not be implemented as a fourth `IGenerationMode`, and `FileGenerationRequest` shall not be extended for it (ADR-0008). Standard and Production Set output, `ZipArchiveSink`, `SourcePathSanitizer`, `ChaosEngine`, and the Composer → Serializer → Emitter seam shall not be altered to produce intentional Archive defects.
- **REQ-217**: Each Expectation File shall conform to `tests/fixtures/archive-test-case.schema.json` (JSON Schema draft-07): required fields `schemaVersion`, `generatorContractVersion`, `generatorVersion`, `fixtureId`, `caseKey`, `caseRevision`, `expectationRevision`, `seed`, `classification`, `archive`, `entries`, `mutations`, `expectations`, `limits`; entries keyed by unique ordinal with raw local/central name bytes, wire compression method codes (`localHeaderMethod`, `centralDirectoryMethod` as uint16 0..65535), and generator knowledge (`payloadCodec` enum `stored`, `deflate`, `deflate64`, `bzip2`, `unknown` for files, omitted for directories); mutation records on the before-mutation offset basis with before/after hashes and sizes; inline hex capped at 256 bytes (512 hex characters), larger removed ranges referenced by hash. Semantics that draft-07 cannot express (basename ↔ Fixture ID equality, ordinal uniqueness) shall be verified by the test suite.
