...
- **REQ-199**: `--input-csv` and `--directory-template` shall be mutually exclusive. Source File Paths must be relative and safe: no parent-directory segments (`..`), rooted or UNC paths, drive-qualified paths, dot-segments, or invalid characters. Backslashes are normalized to `/` and repeated separators collapsed. Paths must be unique per record after normalization (case-insensitive). Input files and template directories must be located within the working directory and must exist; violations fail validation with a clear error message and non-zero exit code.
...
- **REQ-202**: A Source Record's `ControlNumber` value shall override the generated Control Number for that record. A `BatesNumber` value shall override the Bates sequence value for that record; specifying the BatesNumber column requires `--bates-prefix`, and rows without a Bates value fall back to the Bates sequence.
