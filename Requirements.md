...
- **REQ-199**: `--input-csv` and `--directory-template` shall be mutually exclusive. Source File Paths must be relative and safe: no parent-directory segments (`..`), rooted or UNC paths, drive-qualified paths, dot-segments, invalid characters, reserved Windows device names (`CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9`, `LPT1`-`LPT9`, with or without an extension), or segments ending in a dot or space (unextractable on Windows). Backslashes are normalized to `/` and repeated separators collapsed. Paths must be unique per record after normalization (case-insensitive). Input files and template directories must be located within the working directory and must exist; violations fail validation with a clear error message and non-zero exit code.
