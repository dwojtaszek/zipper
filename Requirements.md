...
- **REQ-212**: Every fixture shall carry a classification of `valid`, `malformed`, or `policy-sensitive`, and its Expectation File shall record per-operation expectations (`list`, `read-entry`, `integrity-check`, `extract`) with named reader profiles, allowed outcomes, allowed failure stages, and invariants. `unsupported` and `not-run` shall not count as passing verifications.
