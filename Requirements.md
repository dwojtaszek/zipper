...
- **REQ-001**: When `--with-metadata` is specified, the output `.dat` file must include the additional columns: `Custodian`, `Date Sent`, `Author`, `File Size`, and `Compression Method` (the Archive-wide `--compression` setting: `Store`, `Deflate`, `Deflate64`, or `BZip2`). Note: For the Email File Type, email-specific metadata columns are always included regardless of this flag, as these are intrinsic to Emails. `Compression Method` is **Email-intrinsic**: it is part of Email Metadata, so an Archive containing at least one Email includes the column whether or not `--with-metadata` is specified. For every other File Type the column is strictly gated on `--with-metadata`. The column is absent from the OPT format, which carries no Metadata columns, and from the Production Set DAT, which uses its own fixed schema.
