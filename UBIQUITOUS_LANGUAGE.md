...
| **Source-Driven Generation** | A generation mode (Standard or Loadfile-Only) where records come from a user-supplied **Source CSV** (`--input-csv`) or **Directory Template** (`--directory-template`) instead of a synthetic plan. Source files define **File Paths**, **File Types**, optional identity overrides, and (Source CSV only) **Source Metadata**; their bytes are never copied. Multiple source **File Types** behave like a **File Type Mix**. | CSV-driven generation, template-driven generation |
| **Source Record** | One row of source input: a **File Path**, a **File Type**, and optional `ControlNumber`, `BatesNumber`, and **Source Metadata** values. Maps to exactly one **Native File** (Standard) or one **Load File** record (Loadfile-Only). | Input row, source row |
