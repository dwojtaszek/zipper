...
2. **Generation**: N concurrent producers generate file data and write to result channel. File payload generation is gated by an in-flight ordering window (`maxInFlight`, default `Math.Max(concurrency * 2, 2)`) to bound retained memory when earlier items are delayed. All file types run in parallel. In a File Type Mix run (`--types`), each `FileWorkItem` carries its own File Type from the `FileTypePlan` and producers route to the matching per-type generator; single-type runs resolve to one generator as before.
3. **Archive writing**: Single consumer (`ZipArchiveSink`, implementing `IArchiveSink`) writes ZIP entries in order, draining out-of-order items into a buffer without blocking channel reads (avoiding deadlocks) and releasing ordering-window permits via `onItemCommitted` as items are written and disposed. Load File emission is delegated to `LoadFileOrchestrator`, which drives the composer → serializer → emitter seam (selected via `ILoadFileWriter`; see [Load File Composition Seam](#load-file-composition-seam)).
4. **Deadlock & Fault protection**: `Task.WhenAny` races consumer with producers; linked cancellation tokens and channel error completion ensure prompt producer unblocking, buffer disposal, and temp file cleanup on consumer faults or pipeline cancellation.
