...
| `--rolling-count`, `--rolling-bates-mode`, `--production-id` | Require `--production-set`. `--bates-prefix` is limited to 200 UTF-8 bytes per element, and `--production-id` is limited to 250 UTF-8 bytes per generated element after rolling Production ID derivation. A `--production-id` value must be non-empty, and every supplied or derived element must be a **Safe Path Segment** — no path separators, dot segments, control characters, trailing space or dot, Windows-reserved characters, or reserved DOS device names — because each element names one directory. When a value breaks several rules at once they are reported in a fixed order: empty value, byte limit, missing `--production-set`, Safe Path Segment, entry count, duplicates, empty list entry. All are validated before output generation. |
