...
| `--input-csv` | none | file path within the working directory | Source-Driven Generation from a Source CSV (mutually exclusive with `--type`, `--types`, `--directory-template`, `--production-set`) |
...
| `--input-csv` / `--directory-template` | Mutually exclusive with each other and with `--type`, `--types`, and `--production-set`. Satisfies the `--type` requirement; `--count` becomes optional but must match the Source Record count when given. Multiple source File Types behave like a File Type Mix (File Type column, per-record Email Metadata and Page Count gating, DAT+OPT default when `jpg`/`tiff` participate). `ControlNumber`/`BatesNumber` columns override per-record identity (BatesNumber requires `--bates-prefix`); extra columns map through `--column-profile` (DAT). Works in Standard and Loadfile-Only modes. `--folders` and `--distribution` are ignored (source paths define the structure) |
