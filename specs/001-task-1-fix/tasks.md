...
### T006 [P] Implement Metadata Column Logic
...
### T007 [P] Implement File Size Calculation for EML
**File**: `Zipper/Program.cs` (GenerateEmlFiles method)
...
### T009 [P] Implement Text File Generation Logic
**File**: `Zipper/Program.cs` (GenerateEmlFiles method)
...
### T013 [P] Update Existing Test Suite Integration
**File**: `tests/run-tests.sh` and `tests/run-tests.bat` (existing)
**Description**: Update both existing test runners to include the new comprehensive EML test suite
**Windows Update**: Add call to `test-eml-comprehensive.bat` in `run-tests.bat`
**Unix Update**: Add call to `test-eml-comprehensive.sh` in `run-tests.sh`
**Validation**: Ensure both comprehensive tests are executed as part of full test suite
...
**Content**: Add examples and clarify column ordering for EML files with cross-platform testing information
