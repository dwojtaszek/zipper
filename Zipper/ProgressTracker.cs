// <copyright file="ProgressTracker.cs" company="PlaceholderCompany">
...
namespace Zipper
{
    ...
    {
        private static long filesCompleted;
        ...
        public static void Initialize(long totalFiles)
        {
            lock (Lock)
            {
                ProgressTracker.totalFiles = totalFiles;
                filesCompleted = 0;
        ...
        {
            Interlocked.Add(ref filesCompleted, count);

            var now = DateTime.UtcNow;
            if ((now - lastProgressUpdate).TotalMilliseconds >= 100) // Update every 100ms
            {
                ReportProgress(filesCompleted, totalFiles);
        ...
        private static double lastDisplayedPercentage = 0;
        ...
        public static void ReportProgress(long completed, long total)
        {
            ...
            if (displayPercentage > lastDisplayedPercentage || actualPercentage >= 100)
            {
        ...
        private static double CalculateDisplayPercentage(double actualPercentage)
        {
            ...
            var nextThreshold = Math.Ceiling(lastDisplayedPercentage / 5.0) * 5.0;
            ...
            if (actualPercentage >= nextThreshold)
            {
                ...
                var randomIncrement = (Random.NextDouble() * maxIncrement) + 0.5; // 0.5% to 5.5%
        ...
        {
            return Interlocked.Read(ref filesCompleted);
        }
